...
export const listsRouter = createTRPCRouter({
  ...
  byUserId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      return getListByUserId(input.userId, ctx.db)
    }),
  ...
  check: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return updateIngredientCheckStatus(input.id, input.checked, ctx.db)
    }),

  checkMany: protectedProcedure
    .input(z.array(z.object({ id: z.string(), checked: z.boolean() })))
    .mutation(async ({ ctx, input }) => {
      const transaction = await updateManyIngredientsCheckStatus(input, ctx.db)
  ...
  clear: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const userId = ctx.session.user.id
      return clearCheckedIngredientsFromList(input, userId, ctx.db)
    })
})
